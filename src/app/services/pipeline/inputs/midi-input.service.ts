...
export class MidiInputService {
    ...
    if ('navigator' in window && 'requestMIDIAccess' in window['navigator']) {
      try {
        window.navigator['requestMIDIAccess']().then(
          (access) => {
            const loopCondition = access.inputs.keys();
            while (true) {
              const result = loopCondition.next();
              if (result.done === true) {
                break;
              }
              // otherwise let's add the input we found
              this.inputs.push({key: result.value, value: access.inputs.get(result.value)});
            }
            // TODO - for now, use only first input, later add dropdown to control panel
            // and connect the one selected on demand
            this.connectDefaultInput();
          });
      } catch (e) {
        this.logger.log('MIDI Access denied, no stairway :(');
      }
    }
  }
  ...
  private connectDefaultInput() {
    if (this.inputs && this.inputs.length > 0) {
      ...
      this.logger.log('no midi inputs');
    }
  }
        ...
        .then(
          (channelInputStream$: any) => {
            channelInputStream$.onmidimessage = (midiChannelData) => {
              const parsedMessage: SynthMessage = this.decode(midiChannelData);
              // some notes don't map to a message, ignore those
              if (parsedMessage) {
                this.synthStream$.next(parsedMessage);
              }
            };
          },
          (error) => {
    ...
    switch (midiChannelMessage.data[0]) {
      case 145:
        if (midiChannelMessage.data[1] === 1) {
          this.logger.log('mod level set to to', midiChannelMessage.data[2]);
          // no response
          ...
          this.logger.dir(midiChannelMessage);
        }
        break;
      case 144:
      ...
      default:
        ...
        this.logger.log('invalid midiChannelMessage?', midiChannelMessage);
    }
    return response;
  }
}
