...
export class NoteTranslationService {
  ...
  private defaultOctaves = [3, 4];

  constructor() { }

  getDefaultScaleFrequencies(octaves = this.defaultOctaves) {
    return this.getScaleFrequencies(Object.keys(this.scales)[0], octaves);
  }

  getFrequency(note: string): number {
    return this.noteMappings[note];
  }

  getScaleFrequencies(scale, octaves = this.defaultOctaves) {
    let freqs = [];
    octaves.forEach((curOctave) => {
      freqs = freqs.concat(this.scales[scale].map((curNote) => {
        return this.noteMappings[curNote + curOctave];
      }));
    });
    return freqs;
  }
